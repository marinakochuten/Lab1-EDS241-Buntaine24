...
exp(-0.38)-1  # find percent change, the model estimates a 31% reduction in waste piles
...
# convert coefficient from log scale to percent change for interpretation
exp(-.42)-1 # the treatment has 34% less waste piles
```
...
m4_control <- lm(
    log(waste_piles) ~ 
        treat + 
        pre_count_avg, ## Add control
    data = post_treat_subset)

summ(m4_control, model.fit = FALSE)

# my values here much different that adam's example
...
m5_control2 <- lm(
    log(waste_piles) ~ 
        treat +
        log(pre_count_avg) + 
        rain_0_48hrs + rain_49_168hrs, ## Add control
    data = post_treat_subset)

summ(m5_control2,
     model.fit = FALSE, 
     robust = "HC2")  ## Add heteroskedasticity robust standard error adj.
```
